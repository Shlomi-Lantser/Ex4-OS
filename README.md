# Ex4-OS
Server stack proccess TCP communication.

## Or Cohen & Shlomi Lantser.

# Description

In this project we implemented a stack of strings to put it on our server , the server supports multiproccessing by `fcntl()` file lock method
the server store the stack that we implemented and every client has it own proccess.
The client sending command to the server that changing the stack to all the other clients , the file lock method `fcntl()` inspectoring that every method of the stack 
